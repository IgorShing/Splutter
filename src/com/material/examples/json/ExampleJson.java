...
public class ExampleJson implements Demo {
	...
	void readFromJson() {
		/*
		 * User user = new User(); GovernmentAgency agency = new
		 * GovernmentAgency();
		 * 
		 * ObjectMapper mapper = new ObjectMapper(); try { //
		 * mapper.writeValue(new
		 * File("..\\material\\examples\\json\\data\\user.json"), user); // user
		 * = mapper.readValue(new File("user1.json"), User.class); // agency =
		 * mapper.readValue(new File("govAgency1.json"),
		 * GovernmentAgency.class);
		 * 
		 * // display to console //
		 ...
		 * 
		 * String strJson =
		 * "{\"name\":\"Agency\",\"searchTerms\":[\"law\",\"law\",\"law\",\"justice\",\"judge\",\"decision\"]}"
	...
	void writeToJson() {
		 ...
		 * 
		 * searchTerms.add("law"); searchTerms.add("justice");
		 * searchTerms.add("judge"); searchTerms.add("decision");
		 * 
		 * agency.setSearchTerms(searchTerms);
		 * 
		 * ObjectMapper mapper = new ObjectMapper(); try { //
		 * mapper.writeValue(new
		 * File("..\\material\\examples\\json\\data\\user.json"), user); //
		 * mapper.writeValue(new File("user1.json"), user);
		 * //mapper.writeValue(new File("govAgency1.json"), agency);
		 * 
		 * // display to console // String strForUser = mapper.toString(); //
